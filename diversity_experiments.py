...
from tracin.tracin import big_run, save_tracin_checkpoint, load_tracin_checkpoint, calculate_tracin_influence, run_experiments
import pandas as pd
...
curr_dir = os.getcwd()
user_filter_stats_file = curr_dir + '/user_comparisons/filter_stats.csv'
filter_moderate_file = curr_dir + '/user_comparisons/filter_moderate.csv'
moderate_filter_file = curr_dir + '/user_comparisons/moderate_filter.csv'
diverse_filter_file = curr_dir + '/user_comparisons/diverse_filter.csv'
filter_diverse_file = curr_dir + '/user_comparisons/filter_diverse.csv'
moderate_diverse_file = curr_dir + '/user_comparisons/moderate_diverse.csv'
diverse_moderate_file = curr_dir + '/user_comparisons/diverse_moderate.csv'
full_diversity_file = curr_dir + '/user_comparisons/full_diversity.txt'
...
all_users = list(set([o[0] for o in original_data]))

raw_influences = {}
scores= {"filter_bubble":[], "diverse":[], "moderate": []}
for i in tqdm(range(len(all_users)), desc="Users"):
    user = all_users[i]
    # Subsetting Data for user 
    user_data = np.array([o for o in original_data if o[0]==user])
    # Get data for the user
    (train,test, train_items, test_items) = sequence_generator(user_data,look_back)
    ...
    raw_influences[user] = total_diversity
    total_filter_bubbles = sum([1 if i ==-1 else 0 for i in total_diversity ])
    total_diverse = sum([1 if i ==1 else 0 for i in total_diversity ])
    total_moderate = sum([1 if i ==0 else 0 for i in total_diversity ])
    filter_df = pd.DataFrame(data={'num_filter_bubbles': [total_filter_bubbles], 'num_diverse':[total_diverse], 'num_moderate':[total_moderate] })

    if user == 0:
        filter_df.to_csv(user_filter_stats_file, mode="w+", header=True, index=False)
    else:
        filter_df.to_csv(user_filter_stats_file, mode="a+", header=False, index=False)
...
# TracIn section Evaluating points against each other
    ...
    # Tab everything below
    if len(filter_bubbles) != 0 and len(diverse_points) != 0 and len(moderate_points) != 0:
        # All possible combinations
        # Filter Moderate
        big_run(filter_moderate_file, user, LSTM, filter_bubbles, filter_bubbles_labels, moderate_points, moderate_points_labels, checkpoints, device, optimizer="SGD")
        # Moderate Filter
        big_run(moderate_filter_file, user, LSTM, moderate_points, moderate_points_labels, filter_bubbles, filter_bubbles_labels, checkpoints, device, optimizer="SGD")
        # Diverse Filter
        big_run(diverse_filter_file, user, LSTM, diverse_points, diverse_points_labels, filter_bubbles, filter_bubbles_labels, checkpoints, device, optimizer="SGD")
        # Filter Diverse
        big_run(filter_diverse_file, user, LSTM, filter_bubbles, filter_bubbles_labels, diverse_points, diverse_points_labels, checkpoints, device, optimizer="SGD")
        # Diverse Moderate
        big_run(diverse_moderate_file, user, LSTM, diverse_points, diverse_points_labels, moderate_points, moderate_points_labels, checkpoints, device, optimizer="SGD")
        # Moderate Diverse
        big_run(moderate_diverse_file, user, LSTM, moderate_points, moderate_points_labels, diverse_points, diverse_points_labels, checkpoints, device, optimizer="SGD")
    elif len(diverse_points) == 0 and len(filter_bubbles) != 0 and len(moderate_points) != 0:
        #Filter Moderate
        big_run(filter_moderate_file, user, LSTM, filter_bubbles, filter_bubbles_labels, moderate_points, moderate_points_labels, checkpoints, device, optimizer="SGD")
        # Moderate Filter
        big_run(moderate_filter_file, user, LSTM, moderate_points, moderate_points_labels, filter_bubbles, filter_bubbles_labels, checkpoints, device, optimizer="SGD")
...
with open(full_diversity_file, 'w+') as f:
    json.dump(raw_influences, f)
